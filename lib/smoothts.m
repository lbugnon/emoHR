...
%   $Revision: 1.1.6.5 $   $Date: 2012/08/21 00:14:33 $
...
otherwise
    error(message('finance:ftseries:ftseries_smoothts:InvalidNumberOfInputArguments'));
end

if any(double(wsize < 0)) | (prod(size(wsize)) ~= 1)
    error(message('finance:ftseries:ftseries_smoothts:WSIZEMustBePosScalar'));
...
case 'b'
    ...
    elseif nargin == 4
        error(message('finance:ftseries:ftseries_smoothts:TooManyInputArguments'));
    end
...
otherwise
    error(message('finance:ftseries:ftseries_smoothts:InvalidSmoothingMethod'));
end
